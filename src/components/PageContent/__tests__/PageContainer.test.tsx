...
jest.mock('@mui/material', () => {
  return {
    Box: ({ children, sx, mt, mb, ...props }: any) => {
      ...
      if (mt) style.marginTop = typeof mt === 'number' ? `${mt * 8}px` : mt;
      if (mb) style.marginBottom = typeof mb === 'number' ? `${mb * 8}px` : mb;
      
      // Add any sx styles if provided
      if (sx) {
        if (sx.mt) style.marginTop = typeof sx.mt === 'number' ? `${sx.mt * 8}px` : sx.mt;
        if (sx.mb) style.marginBottom = typeof sx.mb === 'number' ? `${sx.mb * 8}px` : sx.mb;

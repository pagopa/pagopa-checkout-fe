...
describe('DisclaimerField', () => {
    ...
    it('renders HTML message correctly', () => {
        ...
        const paragraph = screen.getByText((content) => {
            return content.includes('Proseguendo');
        });
        ...
        expect(paragraph.querySelector("a[href='https://www.conventionalcommits.org/en/v1.0.0/']")).toBeInTheDocument();
        expect(paragraph.querySelector("a[href='https://www.conventionalcommits.org/en/v1.0.0/']")).toBeInTheDocument();

        const terminiLink = screen.getByRole('link', { name: /termini e condizioni/i });
        expect(terminiLink).toBeInTheDocument();
        expect(terminiLink).toHaveAttribute('href', 'https://www.conventionalcommits.org/en/v1.0.0/');
        ...
        expect(privacyLink).toHaveAttribute('href', 'https://www.conventionalcommits.org/en/v1.0.0/');

  ...
  useTranslation: () => ({
    t: (key: string) => {
      const translations: Record<string, string> = {
        'findOutMoreModal.title': 'Cosa fare se il pagamento non va a buon fine?',
        'findOutMoreModal.section1Title': '1. Paga l\'avviso',
        'findOutMoreModal.section1ContentRow1': 'Ricorda di pagare l\'avviso entro le scadenze previste dall\'ente.',
        'findOutMoreModal.section1ContentRow2': 'Se non riesci tramite questo sito, ',
        'findOutMoreModal.section1ContentRow2Link': 'scopri gli altri canali abilitati a pagoPA.',
        'findOutMoreModal.section2Title': '2. Attendi il rimborso',
        'findOutMoreModal.section2ContentRow1': 'Di solito l\'importo viene riaccreditato entro pochi minuti.',
        'findOutMoreModal.section2ContentRow2': 'In altri casi, il trasferimento di denaro sul tuo conto o carta potrebbe richiedere più tempo.',
        'findOutMoreModal.section3Title': '3. Contatta l\'assistenza',
        'findOutMoreModal.section3ContentRow1': 'Se dopo 5 giorni lavorativi non hai ancora ricevuto il rimborso, ',
...
describe('FindOutMoreModal Component', () => {
  ...
  it('renders correctly when open', () => {
    ...
    expect(screen.getByText('Cosa fare se il pagamento non va a buon fine?')).toBeInTheDocument();
    ...
    expect(screen.getByText('Ricorda di pagare l\'avviso entro le scadenze previste dall\'ente.')).toBeInTheDocument();
    expect(screen.getByText('Se non riesci tramite questo sito,')).toBeInTheDocument();
    expect(screen.getByText('Di solito l\'importo viene riaccreditato entro pochi minuti.')).toBeInTheDocument();
    expect(screen.getByText('In altri casi, il trasferimento di denaro sul tuo conto o carta potrebbe richiedere più tempo.')).toBeInTheDocument();
    expect(screen.getByText('Se dopo 5 giorni lavorativi non hai ancora ricevuto il rimborso,')).toBeInTheDocument();
    
    // Check if links are rendered with correct href attributes
    const paymentMethodsLink = screen.getByText('scopri gli altri canali abilitati a pagoPA.');
    expect(paymentMethodsLink).toBeInTheDocument();
    expect(paymentMethodsLink).toHaveAttribute('href', 'https://www.pagopa.gov.it/it/cittadini/dove-pagare/');
    expect(paymentMethodsLink).toHaveAttribute('target', '_blank');
    
    const supportLink = screen.getByText('contatta l\'assistenza.');
    expect(supportLink).toBeInTheDocument();
    expect(supportLink).toHaveAttribute('href', 'https://www.pagopa.gov.it/it/assistenza/');

...
describe("DrawerCart Component", () => {
  ...
  it("renders payment notices correctly", () => {
    ...
    const amount1Elements = amountElements.filter(
      (el) => el.textContent === "€100.50"
    );
    const amount2Elements = amountElements.filter(
  ...
  it("expands an accordion when clicked", () => {
    ...
    return waitFor(() => {
      ...
      const visibleAmountElement = amountElements.find((el) => {
        ...
        return (
          collapseParent &&
          !collapseParent.classList.contains("MuiCollapse-hidden")
        );
      });
  ...
  it("hides notice number and fiscal code in WISP_REDIRECT mode on specific routes", async () => {
    ...
    jest
      .spyOn(sessionStorage, "getSessionItem")
      .mockImplementation(() => "WISP_REDIRECT");
  ...
  it("truncates long descriptions and company names in accordion summary", () => {
    const longTextNotice: PaymentNotice = {
      amount: 100.5,
      description:
        "This is a very long description that should be truncated in the summary view",
      companyName:
        "This is a very long company name that should also be truncated in the summary",
      noticeNumber: "123456789",
      fiscalCode: "ABCDEF12G34H567I",
    };
  ...
  it("shows full text in accordion details", () => {
    const longTextNotice: PaymentNotice = {
      amount: 100.5,
      description:
        "This is a very long description that should not be truncated in the details view",
      companyName:
        "This is a very long company name that should not be truncated in the details view",
      noticeNumber: "123456789",
      fiscalCode: "ABCDEF12G34H567I",
    };
    ...
    expect(
      screen.getByText(longTextNotice.description as string)
    ).toBeInTheDocument();
    expect(
      screen.getByText(longTextNotice.companyName as string)
    ).toBeInTheDocument();
  });
  ...
  it("has correct accessibility attributes", () => {
    ...
    expect(secondAccordionButton).toHaveAttribute(
      "aria-controls",
      "paynotice-1"
    );
